...
public class ListeEvenementSponsorController extends FrontNavController {
    ...
    void initialize() {
        ...
        sortCombo.getItems().addAll("Date (Plus recent)", "Date (Plus ancien)", "Prix (Croissant)", "Prix (Decroissant)", "Titre (A-Z)");
        sortCombo.setValue("Date (Plus recent)");
    ...
    private void filtrerEtAfficher() {
        ...
        if (sortOption != null) {
            switch (sortOption) {
                case "Date (Plus recent)":
                    filteredList.sort(Comparator.comparing(e -> normalize(e.getDate()), Comparator.reverseOrder()));
                    break;
                case "Date (Plus ancien)":
                    filteredList.sort(Comparator.comparing(e -> normalize(e.getDate())));
                    break;
                case "Prix (Croissant)":
                ...
                case "Prix (Decroissant)":
                    ...
                    break;
                case "Titre (A-Z)":
                    filteredList.sort(Comparator.comparing(e -> normalize(e.getTitre())));
                    break;
            }
        }

        if (filteredList.isEmpty()) {
            Label noMatch = new Label("Aucun evenement a sponsoriser ne correspond a vos criteres.");
    ...
    private VBox createEventCard(Evenement e) {
        ...
        badge.setStyle("-fx-background-color: #f1c40f; -fx-text-fill: white; -fx-font-size: 11px; -fx-font-weight: bold; -fx-padding: 4 10; -fx-background-radius: 10;");
        ...
        prix.setStyle("-fx-font-size: 22px; -fx-font-weight: bold; -fx-text-fill: #27ae60;");

        HBox actions = new HBox(15);
        actions.setAlignment(Pos.CENTER_LEFT);
        actions.setPadding(new Insets(10, 0, 0, 0));
        Button viewBtn = new Button("Sponsoriser");
        viewBtn.setStyle("-fx-background-color: #f39c12; -fx-text-fill: white; -fx-font-weight: bold; -fx-cursor: hand; -fx-padding: 8 20; -fx-background-radius: 20;");
    ...
    private void allerVersDetails(Evenement e) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/views/front/details-evenement-sponsor.fxml"));
