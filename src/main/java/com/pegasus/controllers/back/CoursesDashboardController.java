...
public class CoursesDashboardController {
    ...
    private void openCourseSectionsWindow(Course course) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/views/back/CourseSectionsContent.fxml"));
    ...
    private void openQuizQuestionsWindow(Quiz quiz) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/views/back/QuizQuestionsContent.fxml"));
            Parent root = loader.load();
    ...
    private void showCourseDialog(Course courseToEdit) {
        ...
        VBox content = buildDialogContent(
                ...
                courseToEdit == null ? "Submit a new course as draft for admin approval." : "Update the selected course details.",
    ...
    private void saveCourseFromDialog(Course courseToEdit, String title, String description, String thumbnail,
                                      String status) {
        String statusToSave = resolveArtistCourseStatus(courseToEdit, status);
        String error = validateCourseInputs(title, description, statusToSave);
        if (error != null) {
        ...
        try {
            if (courseToEdit == null) {
                courseService.add(new Course(title, description, thumbnail.isEmpty() ? null : thumbnail, "DRAFT", LocalDateTime.now(), currentArtistId));
            } else {
                ...
                courseToEdit.setStatus(statusToSave);
    ...
    private String resolveArtistCourseStatus(Course courseToEdit, String requestedStatus) {
        if (courseToEdit == null) {
            return "DRAFT";
        }
        ...
        if (courseToEdit.getStatus() != null && !courseToEdit.getStatus().isBlank()) {
            return courseToEdit.getStatus();
        }
        return "DRAFT";
    }
