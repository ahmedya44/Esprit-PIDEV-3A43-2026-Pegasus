...
public class ListeEvenementArtisteController extends FrontNavController {
    ...
    private void filtrerEtAfficher() {
        ...
        if (filteredList.isEmpty()) {
            Label noMatch = new Label("Aucun evenement ne correspond a vos criteres.");
    ...
    private VBox createEventCard(Evenement e) {
        ...
        card.setStyle("-fx-background-color: white; -fx-background-radius: 15; -fx-effect: dropshadow(three-pass-box, rgba(0,0,0,0.1), 15, 0, 0, 5);");
    ...
    private void allerVersModifier(Evenement e) {
        try {
    ...
    private void allerVersParticipants(Evenement e) {
        try {
    ...
    private void supprimerEvenement(Evenement e) {
        ...
        alert.setHeaderText("Supprimer l'evenement : " + e.getTitre() + " ?");
    ...
    @FXML
    void allerVersGestion(javafx.event.ActionEvent event) throws IOException {
        FXMLLoader loader = new FXMLLoader(getClass().getResource("/views/front/ajouter-evenement.fxml"));
        Parent root = loader.load();
