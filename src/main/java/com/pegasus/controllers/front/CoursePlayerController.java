...
public class CoursePlayerController implements FrontContentAware {
    ...
    @FXML
    private StackPane introBackdrop;

    @FXML
    private StackPane courseIntroStage;

    @FXML
    private VBox courseIntroCard;
    ...
    public void initialize() {
        ...
        if (courseIntroCard != null) {
            courseIntroCard.setOpacity(0);
            courseIntroCard.setScaleX(0.94);
            courseIntroCard.setScaleY(0.94);
        }
    ...
    private void startCourse() {
        ...
        if (btnStartCourse != null) {
            btnStartCourse.setText("Opening sections...");
            btnStartCourse.setDisable(true);
        }
        if (courseIntroStage != null) {
            courseIntroStage.setManaged(false);
            courseIntroStage.setVisible(false);
        }
        ...
        Platform.runLater(() -> playerScrollPane.setVvalue(0.0));
        refreshSectionsList();
        updatePlayerState();
    }

    public void startCourseFromPreview() {
        if (course != null) {
            startCourse();
        }
    }

    private void animatePlayerArea() {
        if (playerArea == null) {
            return;
        }
        playerArea.setOpacity(0);
        playerArea.setTranslateY(14);

        FadeTransition fade = new FadeTransition(Duration.millis(220), playerArea);
        fade.setToValue(1);
        TranslateTransition slide = new TranslateTransition(Duration.millis(260), playerArea);
        slide.setToY(0);
    ...
    private void refreshCourseData() {
        ...
        if (courseIntroStage != null) {
            courseIntroStage.setManaged(true);
            courseIntroStage.setVisible(true);
        }
        playerArea.setManaged(false);
        playerArea.setVisible(false);
    ...
    private void renderHeroThumbnail() {
        ...
        if (introBackdrop != null) {
            introBackdrop.getChildren().clear();
        }
    ...
    private void renderIntroBackdrop(Image image) {
        if (introBackdrop == null || image == null) {
            return;
        }

        ImageView backdropImage = new ImageView(image);
        backdropImage.setPreserveRatio(false);
        backdropImage.fitWidthProperty().bind(introBackdrop.widthProperty());
        backdropImage.fitHeightProperty().bind(introBackdrop.heightProperty());
        backdropImage.setEffect(new GaussianBlur(30));
        backdropImage.setOpacity(0.16);
        introBackdrop.getChildren().setAll(backdropImage);
    }

    private void animateIntroCard() {
        if (courseIntroCard == null) {
            return;
        }
        ...
        FadeTransition fade = new FadeTransition(Duration.millis(220), courseIntroCard);
        fade.setToValue(1);
        ScaleTransition scale = new ScaleTransition(Duration.millis(280), courseIntroCard);
        scale.setToX(1);
        scale.setToY(1);
